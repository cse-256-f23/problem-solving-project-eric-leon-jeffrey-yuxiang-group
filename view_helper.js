...
function define_new_effective_permissions(id_prefix, add_info_col = false, which_permissions = null){
    ...
    for(let p of which_permissions) {
        ...
        if(add_info_col) {
            ...
            <td id="${id_prefix}_${p_id}_info_cell" width="32px" style="text-align:right">
                <span id="${id_prefix}_${p_id}_info_icon" class="fa fa-info-circle perm_info" permission_name="${p}" setting_container_id="${id_prefix}"
                onClick="show_permission_dialog(this,'${p}')"/>
            </td>`)
        }
...
function show_permission_dialog(cell,perm){
    let permission_dialog = define_new_dialog("perm_dialog", title='Permissions', options = {});
    //let permission_dialog = document.getElementById("perm_dialog");
    ...
    let my_file_obj_var = path_to_file[currentFilePath];
    let my_user_obj_var = all_users[selectedUsername];
    let my_user_perm = document.getElementById(cell.id).getAttribute("permission_name");
    
    let allowResult = allow_user_action(my_file_obj_var,my_user_obj_var,perm,true);
    
    permission_dialog.empty();
    permission_dialog.append(get_explanation_text(allowResult));
}
    ...
    let perm_list= $(`
        <table id="${id_prefix}" class="ui-widget-content" width="100%">
            <tr id="${id_prefix}_header">
                ...
                <th id="${id_prefix}_header_name">Name</th>
                <th id="${id_prefix}_header_permission">Permission</th>
                <th id="${id_prefix}_header_inherited">Parent Group</th>
...
$('#filestructure').after('<div id="sidepanel" style="display:inline-block;width:250px;margin:20px;"></div>')