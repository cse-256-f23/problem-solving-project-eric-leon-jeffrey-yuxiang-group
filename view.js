...
let new_effective_permission_panel = define_new_effective_permissions(
  "effective_permission_panel",
  true,
  null
);
let user_select_field = define_new_user_select_field(
  ...
  function (selected_user) {
    ...
    $("#effective_permission_panel").attr(
      "filepath",
      "/C/presentation_documents/important_file.txt"
    );
  }
);
let permission_dialog = define_new_dialog(
  "perm_dialog",
  (title = "Permissions"),
  (options = {})
);
...
notes.forEach(function (line) {
  important_note_div.append(
    '<p style="margin-top: 10px; font-size: larger;">' + line + "</p>"
  );
});
...
// append div to sidepanel div
$("#sidepanel").append(important_note_div);
