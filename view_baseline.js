...
grouped_permissions = define_grouped_permission_checkboxes(
  "permdialog_grouped_permissions"
);

let clickTracker = false;
grouped_permissions.on("click", function () {
  clickTracker = true;
...
cant_remove_dialog = define_new_dialog(
  "cant_remove_inherited_dialog",
  "Security",
  {
    buttons: {
      OK: {
...
let are_you_sure_dialog = define_new_dialog(
  "are_you_sure_dialog",
  "Are you sure?",
  {
    buttons: {
...
leftColumn.append(
  $("<p/>", {
    html:
      '<p class="inheritable-hint">If a checkbox is unclickable, go to <strong>Advanced Permission Settings</strong>, uncheck <strong>Inhertiable Permission</strong>, and choose <strong style="color: #4caf50;">Convert</strong></p>',
  })
);
...
function make_all_users_list(id_prefix, attr_set_id, height = 80) {
  let all_user_list = $(
    `<div id="${id_prefix}_all_users" class="selectlist section" style="height:${height}px;overflow-y:scroll"></div>`
  );
      ...
      emitter.dispatchEvent(
        new CustomEvent("userEvent", {
          detail: new ClickEntry(
            ...
            "user dialog: select user " + ui.selected.getAttribute("username"),
            new Date().getTime()
          ),
        })
      );
...
function open_advanced_dialog(file_path) {
  ...
  let all_user_list = make_all_users_list(
    "adv_owner_",
    "adv_owner_current_owner"
  );
...
function update_effective_user() {
  $(".effectivecheckcell").empty();
  let selected_username = $("#adv_effective_current_user").attr(
    "selected_user"
  );
...
function open_user_select(to_populate) {
  ...
  user_select_list = make_all_users_list(
    "user_select",
    "user_select_dialog",
    200
  );
...
effective_user_observer.observe(
  document.getElementById("adv_effective_current_user"),
  { attributes: true }
);
...
for (let p of Object.values(permissions)) {
  ...
  for (let ace_type of ["allow", "deny"]) {
    row.append(
      `<td id="perm_entry_row_${p}_${ace_type}" class="perm_entry_checkcell" perm="${p}" type="${ace_type}"></td>`
    );
  }
...
perm_entry_user_observer = new MutationObserver(function (
  mutationsList,
  observer
) {
  for (let mutation of mutationsList) {
    if (mutation.type === "attributes") {
      if (mutation.attributeName === "selected_user") {
        ...
        $(".perm_entry_checkcell").each(function (i) {
          ...
          let checkbox = $(
            `<input type="checkbox" id="${cell_id}_checkbox" class="perm_entry_checkbox"></input>`
          );
          $(this).append(checkbox);
        });

        let all_perms = get_total_permissions(
          file_obj,
          $("#perm_entry_username").attr("selected_user")
        );
        for (let ace_type in all_perms) {
          for (let p in all_perms[ace_type]) {
            let checkbox = $(
              document.getElementById(
                `perm_entry_row_${p}_${ace_type}_checkbox`
              )
            );
...
perm_entry_user_observer.observe(
  document.getElementById("perm_entry_username"),
  { attributes: true }
);
